...
Disable the built-in screenshot action. On Windows, guest display windows are
also excluded from operating-system capture while a guest display is ready.
Connection and authentication windows are not excluded. This option cannot
protect against privileged capture software, cameras, or keyloggers; use a
trusted host and operating-system security controls for those threats.
